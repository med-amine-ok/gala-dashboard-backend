    ...
    <div className="space-y-8 text-[#1A1A1A]" onClick={handleContainerClick}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`p-6 sm:p-8 rounded-3xl border flex flex-col items-center justify-center text-center h-72 sm:h-80 transition-all shadow-[0_4px_24px_-4px_rgba(26,26,26,0.02)] relative overflow-hidden ${
            ...
            <div className="absolute top-4 left-4 sm:top-5 sm:left-5 flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-semibold opacity-70">
            ...
            {lastScanResult.status === 'success' && (
              <div className="space-y-3 sm:space-y-4 animate-scale-up">
                <CheckCircle2 className="h-12 w-12 sm:h-16 sm:w-16 text-[#2E5A36] mx-auto" />
                <div>
                  <h3 className="text-xl sm:text-2xl font-serif font-bold uppercase tracking-wider">{lastScanResult.message}</h3>
                  <p className="text-sm font-semibold text-gray-700 mt-1 sm:mt-2">{lastScanResult.attendee}</p>
                  <p className="text-[10px] text-gray-500 font-mono mt-0.5 sm:mt-1">Serial: {lastScanResult.serial}</p>
                </div>
              </div>
            )}
            ...
            {lastScanResult.status === 'error' && (
              <div className="space-y-3 sm:space-y-4 animate-scale-up">
                <XCircle className="h-12 w-12 sm:h-16 sm:w-16 text-[#8B2635] mx-auto" />
                <div>
                  <h3 className="text-xl sm:text-2xl font-serif font-bold uppercase tracking-wider">Access Denied</h3>
                  <p className="text-xs sm:text-sm font-medium mt-1 sm:mt-2">{lastScanResult.message}</p>
                </div>
              </div>
            )}

            {lastScanResult.status === null && (
              <div className="space-y-3 sm:space-y-4 text-[#8C8C8C]">
                <div className="h-14 w-14 sm:h-16 sm:w-16 mx-auto rounded-full bg-[#FAF8F5] border border-[#EAE3D5] flex items-center justify-center">
                  <Scan className="h-7 w-7 sm:h-8 sm:w-8 text-[#C5A880] animate-pulse" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-serif font-semibold text-[#1A1A1A]">{lastScanResult.message}</h3>
          ...
          {/* Form input */}
          <form onSubmit={handleScanSubmit} className="bg-white p-4 sm:p-6 border border-[#EAE3D5] rounded-3xl shadow-sm">
            ...
            <div className="mt-3 flex flex-col sm:flex-row gap-2 sm:items-center justify-between text-[11px] text-[#8C8C8C]">
              ...
              <button
                type="submit"
                className="px-4 py-2.5 bg-[#ECE5F8] text-[#6E4FA0] border border-[#DDD0F3] hover:bg-[#DDD0F3] rounded-xl text-xs font-semibold cursor-pointer shadow-2xs transition-all w-full sm:w-auto"
              >
                Validate Manually
              </button>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-[#EFE8DC] p-4 sm:p-6 flex flex-col h-[400px] sm:h-[500px] shadow-sm">
